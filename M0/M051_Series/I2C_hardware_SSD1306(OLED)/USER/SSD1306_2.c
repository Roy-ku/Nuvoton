...
int main(void)
{
  ...
  strcat(I2C_C,I2C_Clock);
	...
	SSD1306_DrawBMP(32,0,96,8,BMP2);
	Delay_ms(1000);
	SSD1306_Clear();
	SSD1306_ShowStr(0,0,CPU,2);
	SSD1306_ShowStr(0,2,I2C_C,2);
	extern const unsigned char F16x16_TC_Str_1[];
	SSD1306_ShowTC(0*16,4,F16x16_TC_Str_1,0);
	SSD1306_ShowTC(1*16,4,F16x16_TC_Str_1,1);
	SSD1306_ShowTC(2*16,4,F16x16_TC_Str_1,2);
	SSD1306_ShowTC(3*16,4,F16x16_TC_Str_1,3);
	SSD1306_ShowTC(4*16,4,F16x16_TC_Str_1,4);
	...
	{		
		...
		SSD1306_ShowStr(64,6,Count_num,2);
		count++;
		Delay_ms(1000);
